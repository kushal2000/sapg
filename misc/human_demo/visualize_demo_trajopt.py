...
def set_keypoint_sphere_positions(hand_keypoint_to_xyz: dict, server: viser.ViserServer) -> None:
    from misc.human_demo.colors import RED_TRANSLUCENT_RGBA, RED_RGBA, GREEN_TRANSLUCENT_RGBA, GREEN_RGBA, BLUE_TRANSLUCENT_RGBA, BLUE_RGBA, YELLOW_TRANSLUCENT_RGBA, YELLOW_RGBA, MAGENTA_RGBA, BLACK_RGBA, CYAN_RGBA
    keypoint_to_rgba = {
...
def save_to_file(file_path: Path, q_array: np.ndarray, object_pose_array: np.ndarray, dt: float):
    ...
    from misc.recorded_data_scripts.recorded_data_sharpa import RecordedData
...
def solve_trajopt(T_R_Ps: np.ndarray, q_start: np.ndarray, dt: float, waypoint_buffer: int = 0) -> np.ndarray:
    from misc.pyroki_tests import pyroki_snippets as pks
    from misc.pyroki_tests.trajopt_improved_waypoints_sharpa import xyzw_to_wxyz
    import pyroki as pk
